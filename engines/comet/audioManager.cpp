...
namespace Cometengine {
	...
	AudioManager::AudioManager(CometEngine* vm) : _vm(vm),
		//_musicData(nullptr), _digiSampleData(nullptr),
													  //_currentMusicFile(0),
		_sbdigi(nullptr),
	...
	void AudioManager::playMusicSubfile(uint16 subfile) {
		...
		if (tmp == -1) {
			...
			/*
				...
				*/
			_vm->_archMgr->getFile("MUS.PAK", subfile, _musicData.get());
			_vm->_gameState.currentMusicFile = subfile;
			this->musicInitSong(_musicData.get());
	...
	void AudioManager::loadDigiSubfile(uint16 subfile) {
		if (subfile != _currenDigiFile) {
			_vm->_archMgr->getFile("SMP.PAK", subfile, _digiSampleData.get(), &_digiDataSize);
		...
		if (subfile == 0xff) {
			...
			if (_sbdigi->getPlayStatus() == 0) {
				...
				if (_vm->_gameState.digiVolume != 0) {
					_sbdigi->playVOC(_digiSampleData.get(), _digiDataSize, numLoops);
				}
			}
		}
	}
